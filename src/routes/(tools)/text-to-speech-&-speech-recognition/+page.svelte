<script>
	...
	const speak = () => {
		...
		if (!utterance.voice || !selectedVoice || !text) {
			console.log('Invalid input. Please select a voice and enter text.');
			return;
		}
...
<div class="box font-normal text-gray-700 dark:text-gray-400">
	...
	<select bind:value={selectedVoice} class="voiceNames  dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
	...
	<input type="range" class="pitch" name="pitch" bind:value={pitch} min="0.1" max="2" step="0.1" id="pitch" />

	<label for="ratelabel" class="ratelabel">select rate</label>
	<input type="range" class="rate"  name="rate" bind:value={pitch} min="0.1" max="2" step="0.1" id="pitch" />

	<label for="volumelabel" class="volumelabel">select volume</label>
	<input type="range" name="volume" class="volume" bind:value={volume} min="0.1" max="1" step="0.1" id="volume" />
	<button on:click={speak} class="buttonsp  py-2.5 px-5 my-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Text to Speech</button>
	<button on:click={runSpeechRecog} class="buttonsptotext py-2.5 px-5 my-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 " style="border: transparent;padding: 0 0.5rem;">
		Speech to text 
	</button>
	<p id="action"></p>
</div>
	...
	<div class="box1" >
		<div class="p-5 mr-2 relative  dark:text-white">
			<Copy text={text.replace(/<\/?p>/g, '\n')}/>
				<textarea bind:value={text} class="textarea dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 "  />
